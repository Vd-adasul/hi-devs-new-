...
export const ClauseClassifier = Extension.create<ClauseClassifierOptions>({
  ...
  addProseMirrorPlugins() {
    ...
    // Sequential rate-limiting queue to stay under Gemini free-tier 15 RPM
    const queue: Array<{ view: EditorView; hash: string; text: string; sectionHint: string | null }> = []
    ...
    const processQueue = async () => {
      ...
      while (queue.length > 0) {
        ...
        if (inflight.has(task.hash)) continue
        inflight.add(task.hash)
        try {
          const res = await api.post<ClassifyResult>('/agent/classify-clause', {
            clauseText:   task.text,
            contractType: opts.contractType,
            sectionHint:  task.sectionHint ?? undefined,
          })
          task.view.dispatch(task.view.state.tr.setMeta('classifierResult', { hash: task.hash, result: res.data }))
        } catch { /* non-fatal */ }
        finally {
          inflight.delete(task.hash)
        }

        // Wait 4.2 seconds before the next request in the queue to respect the rate limit
        if (queue.length > 0) {
    ...
    const scheduleScan = (view: EditorView) => {
      ...
      timer = setTimeout(() => {
        ...
        view.state.doc.descendants((node, _pos) => {
          ...
          budget--
          plug.pending.add(h)
          queue.push({ view, hash: h, text, sectionHint })
          return false
        })
