  ...
  return (
    ...
    >
      <div
        ...
        className="bg-obsidian-700 rounded-2xl shadow-2xl w-full max-w-md mx-4 flex flex-col max-h-[90vh]"
      >
        {/* Header */}
        <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between">
          <div>
            <h2 className="text-base font-semibold text-white">Send for review</h2>
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded hover:bg-obsidian-800 text-slate-500"
            aria-label="Close"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto">
          {loadingWorkflows ? (
            ...
            <>
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1.5">Workflow</label>
                <select
                  value={effectiveWorkflowId ?? ''}
                  onChange={e => setSelectedWorkflowId(e.target.value || null)}
                  data-testid="send-for-review-workflow"
                  className="w-full h-10 text-sm border border-white/10 rounded-md px-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 bg-obsidian-700"
                >
                  {workflows.map(w => (
                ...
                {effectiveWorkflow?.description && (
                  <p className="text-[11px] text-slate-500 mt-1.5">{effectiveWorkflow.description}</p>
                )}
              </div>

              {/* Reviewer chain preview */}
              <div className="bg-obsidian-900 border border-white/10 rounded-lg p-3">
                <div className="text-[10.5px] uppercase tracking-wider font-semibold text-slate-500 mb-1.5">
                  First reviewer
                </div>
                <div className="text-[13px] text-white inline-flex items-center gap-1.5">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full text-sm border border-white/10 rounded-md px-3 py-2 resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500"
                />
              </div>
