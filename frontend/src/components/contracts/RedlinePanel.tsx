...
const SEVERITY_COLORS: Record<string, string> = {
  low:      'bg-obsidian-800 text-slate-400',
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-2.5 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              {change.reasoning && (
                <p className="text-xs text-slate-500 mt-1 leading-relaxed">{change.reasoning}</p>
              )}
            </div>
          </div>
          <button
            onClick={() => setExpanded(e => !e)}
            className="p-1 text-gray-400 hover:text-slate-400 rounded flex-shrink-0"
          >
  ...
  if (versions.length < 2) {
    return (
      <div className="text-center py-12 text-gray-400">
        ...
        <p className="text-sm font-medium text-slate-500">Upload a counterparty version to analyze redlines</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-obsidian-700 border border-white/10 rounded-xl p-4 space-y-3">
        <p className="text-sm font-semibold text-slate-300">Select versions to compare</p>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-slate-500 font-medium block mb-1">Baseline (our version)</label>
            <select
              value={v1Id}
          ...
          <div>
            <label className="text-xs text-slate-500 font-medium block mb-1">Counterparty redlines</label>
            <select
              value={v2Id}
