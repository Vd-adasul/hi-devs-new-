...
export default function ResearchPage() {
  ...
  return (
    <div className="px-6 py-6 max-w-6xl mx-auto bg-obsidian-700 text-white rounded-xl shadow-sm border border-white/[0.06] mt-4" data-testid="research-page">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Search Panel */}
        <div className="bg-obsidian-900 border border-white/10/60 shadow-sm rounded-xl p-5 flex flex-col gap-4">
          <h3 className="text-sm font-semibold text-white border-b border-white/10 pb-2">Precedent Lookup</h3>
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-obsidian-700 border border-white/14 rounded-lg py-1.5 pl-9 pr-4 text-sm placeholder-gray-400 text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                required
              />
            </div>
          ...
          <div className="flex flex-col gap-3 max-h-80 overflow-y-auto mt-2">
            {results.map((res) => (
              <div key={res.tid} className="p-3 border border-white/10 bg-obsidian-700 rounded-lg text-xs shadow-sm">
                <h4 className="font-bold text-white leading-tight">{res.title}</h4>
        ...
        {/* Memo Viewer */}
        <div className="md:col-span-2 bg-obsidian-900 border border-white/10/60 shadow-sm rounded-xl p-5 flex flex-col gap-4 min-h-[350px]">
          ...
          <div className="flex-1 overflow-y-auto pr-2">
            {memo ? (
              <div className="text-xs text-slate-300 leading-relaxed whitespace-pre-wrap bg-obsidian-700 border border-white/10 rounded-lg p-4 shadow-sm font-sans">
                {memo}
              </div>
            ) : (
              <p className="text-xs text-gray-400 text-center py-24 bg-obsidian-700 border border-white/10 border-dashed rounded-lg">
