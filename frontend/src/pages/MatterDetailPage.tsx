...
export function MatterDetailPage() {
  ...
  const deleteMatter = useMutation({
    mutationFn: () => api.delete(`/matters/${id}`).then(r => r.data),
    onSuccess: () => {
  ...
  return (
    <div className="px-6 py-5 max-w-6xl mx-auto" data-testid="matter-detail-page">
      <Link to="/matters" className="inline-flex items-center gap-1 text-[12px] text-muted-foreground hover:text-white mb-3">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="min-w-0">
          <h1 className="text-xl font-semibold text-white flex items-center gap-2">
            ...
            <span className={
              ...
              (data.status === 'OPEN'     ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                : data.status === 'CLOSED' ? 'bg-obsidian-900 text-slate-400 border-white/10'
                :                            'bg-amber-50 text-amber-700 border-amber-200')
            }>{data.status}</span>
          </h1>
          <div className="text-[12px] text-muted-foreground mt-1 flex items-center gap-2 flex-wrap">
            {data.counterpartyName && <span>Counterparty: <span className="text-white">{data.counterpartyName}</span></span>}
            {data.owner && <span>· Owner: <span className="text-white">{data.owner.name}</span></span>}
          ...
          {data.description && <p className="text-[12px] text-slate-300 mt-2 max-w-3xl">{data.description}</p>}
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <Button
            ...
            onClick={() => {
              if (confirm(`Are you sure you want to delete the matter "${data.name}"?`)) {
                deleteMatter.mutate()
              }
            }}
            data-testid="matter-delete-btn"
            className="gap-1 text-[12px]"
          >
            ...
            <li key={c.id}>
              <Link to={`/contracts/${c.id}`} className="block px-4 py-2 hover:bg-muted/40">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-medium text-[12.5px] text-white truncate">{c.title}</span>
          ...
          {(data.requests ?? []).map(r => (
            <li key={r.id} className="px-4 py-2">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="font-medium text-[12.5px] text-white truncate">{r.title}</span>
          ...
          {(data.threads ?? []).map(t => (
            <li key={t.id} className="px-4 py-2">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-[12.5px] text-white truncate">{t.title}</span>
        ...
        <div className="space-y-6" data-testid="matter-tab-living-state-body">
            ...
            <div className="p-4 bg-red-50 border border-red-200 rounded-xl space-y-3">
              ...
              <div className="space-y-3">
                {activeConflicts.map((c) => (
                  <div key={c.existingClauseId} className="bg-obsidian-700 border border-red-100 rounded-lg p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                      <div className="p-2 bg-obsidian-900 rounded border border-white/[0.06]">
                        <strong className="text-slate-400 block mb-1">Original Clause:</strong>
                        <p className="text-white italic leading-relaxed">{c.existingClauseText}</p>
                      </div>
                      <div className="p-2 bg-indigo-50/40 rounded border border-indigo-100">
                        ...
                        <p className="text-white italic leading-relaxed">{c.incomingClauseText}</p>
                      </div>
                    </div>
                    <div className="flex justify-between items-center flex-wrap pt-1 gap-2 border-t border-white/[0.06]">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <h2 className="text-sm font-semibold text-white">Merged Living State</h2>
            ...
            {(!data.livingState || !data.livingState.mergedClauses || data.livingState.mergedClauses.length === 0) ? (
              ...
              <div className="grid grid-cols-1 gap-3">
                {data.livingState.mergedClauses.map((mc, idx) => {
                  ...
                  return (
                    <div key={idx} className="bg-obsidian-700 border rounded-xl p-3.5 space-y-1.5">
                      <div className="flex justify-between items-center">
                        ...
                        {sourceContract && (
                          <span className="text-[10px] text-slate-500">
                            Source: <strong className="text-slate-300">{sourceContract.title}</strong>
                          </span>
                        )}
                      </div>
                      <p className="text-xs leading-relaxed text-slate-300 whitespace-pre-wrap">{mc.text}</p>
                    </div>
                  )
                })}
              </div>
