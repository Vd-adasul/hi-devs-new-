...
import { Upload, Search, FileText, ChevronRight, SlidersHorizontal, X, Loader2, PenSquare, RefreshCcw, Trash2 } from 'lucide-react'
...
export function ContractsPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/contracts/${id}`).then((r) => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contracts'] })
    },
  })
            ...
            <div className="bg-obsidian-700">
              ...
              {contracts.map((c: any) => (
                ...
                <div
                  ...
                  className="grid grid-cols-[minmax(0,2fr)_120px_160px_100px_80px_64px] gap-4 items-center px-6 py-3.5 border-b border-gray-50 hover:bg-blue-50/40 cursor-pointer transition-colors group"
                >
                  {/* Title + type */}
                  ...
                  <div className="flex justify-end items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                    <button
                      type="button"
                      onClick={() => {
                        if (confirm(`Are you sure you want to delete "${displayTitle(c)}"?`)) {
                          deleteMutation.mutate(c.id)
                        }
                      }}
                      className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-600 p-1 rounded transition-opacity"
